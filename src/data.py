...
class DatasetConfig:
    ...
    # How much the synthetic generator has "collapsed" (0=fresh, 1=fully
    # collapsed)
    collapse_severity: float = 0.5
    # Fraction of training labels replaced with uniform random labels
    # (baseline)
...
def generate_modular_arithmetic(
    ...
    if config.collapse_level > 0:
        train_pairs, train_targets_list = apply_collapse(
            ...
            config.collapse_severity,
            rng,
        )

    # Apply uniform random label noise (baseline ablation, mutually
...
def get_all_conditions(prime: int = 59, seed: int = 42) -> dict:
    ...
    return {
        ...
        "low_collapse": DatasetConfig(
            prime=prime, collapse_level=0.05, collapse_severity=0.3, seed=seed
        ),
        "medium_collapse": DatasetConfig(
            prime=prime, collapse_level=0.15, collapse_severity=0.5, seed=seed
        ),
        "high_collapse": DatasetConfig(
            prime=prime, collapse_level=0.30, collapse_severity=0.7, seed=seed
        ),
        "severe_collapse": DatasetConfig(
            prime=prime, collapse_level=0.50, collapse_severity=0.9, seed=seed
        ),
    }
...
if __name__ == "__main__":
    ...
    for name, config in conditions.items():
        ...
        print(
            f"{name}: train={train_in.shape}, test={test_in.shape}, "
